...
interface MusicLoungeBuilding3DProps {
...
export default function MusicLoungeBuilding3D({ position, isNearby }: MusicLoungeBuilding3DProps) {
  ...
  useFrame(({ clock }, delta) => {
    // Slowly spin the vinyl record
    if (vinylRef.current) {
      vinylRef.current.rotation.y += delta * 1.5;
    }
  ...
  return (
    <group position={position}>
      ...
      <group ref={pedestalRef}>
        ...
        {/* Floating spinning vinyl record */}
        <group ref={vinylRef} position={[0, 1.0, 0]}>
          {/* Vinyl disc */}
      ...
      {isNearby && (
        <>
          <FloatingOrb offset={0} color="#e11d48" />
          <FloatingOrb offset={2.1} color="#ffffff" />
          <FloatingOrb offset={4.2} color="#e11d48" />
        </>
      )}
